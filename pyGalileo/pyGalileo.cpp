...
BOOST_PYTHON_MODULE(pygalileo)
{
	...
	class_<GalileoWrap::GalileoWrap>("SDK", init<>())
		...
		(boost::python::arg("targetID"), boost::python::arg("timeout"), boost::python::arg("password"),
			boost::python::arg("OnConnect") = NULL, boost::python::arg("OnDisconnect") = NULL))
		.def("KeepConnection", &GalileoWrap::GalileoWrap::KeepConnection, (boost::python::arg("flag"), boost::python::arg("maxRetry") = 20))
		.def("GetRetryCount", &GalileoWrap::GalileoWrap::GetRetryCount)
		...
		.def("SendAudioRaw", &GalileoWrap::GalileoWrap::SendAudioRaw)
		.def("EnableGreeting", &GalileoWrap::GalileoWrap::EnableGreeting)
    ...
    enum_<GalileoSDK::GALILEO_RETURN_CODE>("GALILEO_RETURN_CODE")
		...
		.value("INVALIDE_PARAMS", GalileoSDK::INVALIDE_PARAMS)
        ;
